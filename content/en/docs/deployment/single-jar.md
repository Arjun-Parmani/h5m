...
Single-JAR mode is the simplest way to run h5m. The build produces one uber-JAR that contains all dependencies. Combined with SQLite as the database backend, h5m requires only a Java 21 runtime — no PostgreSQL, no message broker, no external services.
...
Requires GraalVM with native-image installed. The resulting binary has no JVM dependency.
