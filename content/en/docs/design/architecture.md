...
Each folder has one or more **NodeGroups**. A NodeGroup contains the actual DAG of computation nodes. Nodes are a single polymorphic entity (`NodeEntity`) with a `type` discriminator — `jq`, `js`, `jsonata`, `ft` (fixed threshold), `rd` (relative difference).
