...
{{< alert color="warning" >}}
DuckDB support is currently commented out in the default `application.properties`. You will need to uncomment the relevant configuration and verify the DuckDB JDBC driver is included in the build before using this backend.
{{< /alert >}}
...
Uncomment these lines (or override via environment variables) to activate DuckDB. Ensure the `duckdb-jdbc` dependency is present in your Maven build — the project's `pom.xml` includes `duckdb-jdbc` at version `1.4.4.0`.
