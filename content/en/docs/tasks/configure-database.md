...
Switch to SQLite by overriding the datasource kind and JDBC URL:
...
To use an in-memory SQLite database (data lost on shutdown — useful for testing):

```bash
export QUARKUS_DATASOURCE_JDBC_URL=jdbc:sqlite::memory:
```

The `H5M_PATH` environment variable is a shorthand in some build profiles for setting the SQLite file path. Check the active profile's configuration for whether it is supported.
...
1. Environment variables (`QUARKUS_DATASOURCE_JDBC_URL=...`)
2. System properties (`-Dquarkus.datasource.jdbc.url=...`)
