...
A **Node** is a computation unit that takes one or more inputs and produces one or more outputs. Nodes are the primary building block of h5m's data processing pipeline. Each node uses one of three supported expression languages:
...
h5m is built on [Quarkus](https://quarkus.io/) and can run as a single JAR with no external services. The default persistence backend is **SQLite**, stored at `~/h5m.db` (overridable via `H5M_PATH`). DuckDB and PostgreSQL are also supported for workloads that need them.
...
- **Language:** Java 21 (Quarkus), TypeScript (frontend)
- **License:** Apache 2.0
