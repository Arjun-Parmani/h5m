...
raw JSON upload
  ...
  {throughput}:.value / 1000000           .metrics.latency.p99_ms
      │
      ▼
  [regression_flag]
  {throughput_mbs}:.value < 40.0
```
...
Nodes with no dependencies on each other may execute in any order (and potentially in parallel in future versions).
...
Each **Folder** has its own independent node graph. Nodes in one folder cannot reference nodes in another. This keeps processing isolated and makes folders self-contained units.

## Polymorphic Nodes

Every node in the graph is the same entity type — `NodeEntity` — but with a different `type` field (`jq`, `js`, `jsonata`). This means:
