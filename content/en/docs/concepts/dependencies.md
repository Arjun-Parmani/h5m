...
# .value       → the jq filter applied to that input
# Produces:    52.4288
```
...
h5m add jq to my-benchmarks regression \
  "{throughput_mbs}:.value < {baseline_mbs}:.value * 0.95"
```
...
[throughput]          .metrics.throughput
  │
  ▼
[throughput_mbs]      {throughput}:.value / 1000000
  │
  ▼
[regression_flag]     {throughput_mbs}:.value < 40.0
```
